...
def admin_page() -> HTMLResponse:
    # /admin is the canonical alternative when enterprise filters classify the
    # generic /auth path too aggressively. Keep /auth for compatibility.
    return HTMLResponse(
        ADMIN_HTML,
        headers={
...
ADMIN_HTML = """<!doctype html><html lang='vi'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><meta name='theme-color' content='#111827'><title>ChatGPT Gateway</title><style>:root{font-family:system-ui,-apple-system,sans-serif;color:#111827;background:#f3f4f6}*{box-sizing:border-box}body{margin:0;padding:18px}.app{max-width:560px;margin:auto}.card{background:#fff;border:1px solid #e5e7eb;border-radius:18px;padding:18px;margin-bottom:14px;box-shadow:0 4px 16px #0000000a}h1{margin:0 0 5px;font-size:24px}h2{font-size:17px;margin:0 0 10px}.muted{font-size:14px;color:#6b7280;line-height:1.5}label{display:block;font-size:13px;font-weight:650;margin:14px 0 6px}input,button{width:100%;min-height:48px;border-radius:12px;font:inherit}input{border:1px solid #d1d5db;padding:12px}button{border:0;background:#111827;color:#fff;font-weight:650;padding:12px;margin-top:10px}button.secondary{background:#e5e7eb;color:#111827}.status{margin-top:12px;padding:12px;background:#f3f4f6;border-radius:12px;font-size:14px;white-space:pre-wrap}.code{margin-top:10px;padding:14px;background:#f9fafb;border-radius:12px;text-align:center;font-size:25px;font-weight:800;letter-spacing:2px}.hidden{display:none}.account{display:flex;justify-content:space-between;padding:12px 0;border-bottom:1px solid #eee}.account:last-child{border:0}.dot{display:inline-block;width:9px;height:9px;border-radius:50%;background:#16a34a;margin-right:7px}</style></head><body><main class='app'><section class='card'><h1>ChatGPT Gateway</h1><div class='muted'>Quản lý tài khoản ChatGPT và gateway · trang quản trị: /admin</div></section><section id='login-card' class='card'><h2>Đăng nhập quản trị</h2><label>Tên đăng nhập</label><input id='username' autocomplete='username'><label>Mật khẩu</label><input id='password' type='password' autocomplete='current-password'><button onclick='login()'>Đăng nhập</button><div id='login-status' class='status hidden'></div></section><section id='dashboard' class='hidden'><section class='card'><h2>Quản trị</h2><div class='status'>Phiên quản trị đang hoạt động.</div><button class='secondary' onclick='logout()'>Đăng xuất</button></section><section class='card'><h2>Provider &amp; Model</h2><div id='providers'>Chưa tải.</div><label>Model</label><input id='model-input' list='model-options' placeholder='Để trống = mặc định' onchange='selectModel(this.value)'><button class='secondary' style='min-height:40px;padding:8px 14px;width:auto;margin-top:8px' onclick='applyModel()'>Áp dụng model</button><datalist id='model-options'></datalist><div class='muted'>Chọn từ danh sách (khi có BAI_API_KEY) hoặc nhập tay model của B.AI.</div><div id='provider-status' class='status'></div></section><section class='card'><h2>Client Keys</h2><div class='muted'>Mỗi API key client có thể dùng provider/model riêng. Key master (GATEWAY_API_KEY) dùng lựa chọn chung phía trên.</div><label>Tên client</label><input id='client-label' placeholder='VD: Bot Zalo'><label>API key</label><input id='client-key' placeholder='Để trống để tự sinh'><label>Provider</label><select id='client-provider'><option value='chatgpt'>ChatGPT / Codex</option><option value='bai'>B.AI</option><option value='openrouter'>OpenRouter</option><option value='tokenrouter'>TokenRouter</option><option value='notion'>Notion AI</option><option value='nim'>NVIDIA NIM</option></select><label>Model</label><input id='client-model' placeholder='Để trống = mặc định'><button onclick='addClient()'>Thêm client</button><button class='secondary' onclick='loadClients()'>Làm mới</button><div id='client-created' class='status hidden'></div><div id='clients' class='status'>Chưa tải.</div></section><section class='card'><h2>Đăng nhập ChatGPT</h2><div class='muted'>Không cần nhập access token. Gateway sẽ dùng Device Login của ChatGPT và lưu refresh token được mã hóa trong database.</div><button onclick='startLogin()'>Đăng nhập ChatGPT</button><div id='device-status' class='status hidden'></div><div id='code' class='code hidden'></div><button id='open' class='secondary hidden'>Mở trang xác nhận</button></section><section class='card'><h2>Tài khoản ChatGPT</h2><button class='secondary' onclick='loadAccounts()'>Làm mới</button><div id='accounts' class='status'>Chưa tải.</div></section><section class='card'><h2>OpenRouter</h2><div class='muted'>Dán OpenRouter API key (sk-or-v1-…) để bật provider OpenRouter. Danh sách model lấy từ OpenRouter và lọc sẵn các model free (đuôi :free). Key được mã hóa và lưu trong database.</div><label>API key</label><input id='openrouter-key' placeholder='sk-or-v1-...' autocomplete='off'><button onclick='saveOpenRouterKey()'>Lưu key</button><div id='openrouter-status' class='status hidden'></div></section><section class='card'><h2>TokenRouter</h2><div class='muted'>Dán TokenRouter API key (tr_…) để bật provider TokenRouter. Gateway dùng API OpenAI-compatible và lấy danh sách model động từ TokenRouter. Key được mã hóa và lưu trong database.</div><label>API key</label><input id='tokenrouter-key' placeholder='tr_...' autocomplete='off'><button onclick='saveTokenRouterKey()'>Lưu key</button><div id='tokenrouter-status' class='status hidden'></div></section><section class='card'><h2>NVIDIA NIM</h2><div class='muted'>Lấy API key (nvapi-…) tại build.nvidia.com rồi dán vào đây. Danh sách model lấy trực tiếp từ NVIDIA, đã lọc bỏ các model không dùng được cho chat (embedding, rerank, OCR…) — phần còn lại là free tier của NIM.</div><label>API key</label><input id='nim-key' placeholder='nvapi-...' autocomplete='off'><button onclick='saveNimKey()'>Lưu key</button><div id='nim-status' class='status hidden'></div></section><section class='card'><h2>Đăng nhập Notion AI</h2><div class='muted'><b>Cách 1 — trình duyệt:</b> nếu gateway đang chạy trên máy có Chrome/Edge và desktop, nhấn nút dưới đây. Gateway mở một cửa sổ trình duyệt tạm, bạn đăng nhập Notion bình thường, sau đó gateway tự lấy session tối thiểu (token_v2, notion_user_id, notion_users và các định danh browser nếu Notion cấp) rồi đóng cửa sổ tạm.</div><button onclick='startNotionBrowserLogin()'>Đăng nhập Notion bằng trình duyệt</button><div id='notion-browser-status' class='status hidden'></div><div class='muted' style='margin-top:14px'><b>Cách 2 — session thủ công:</b> dán các giá trị cookie từ cùng một phiên Notion. <b>token_v2</b> là bắt buộc; <b>notion_user_id</b> và <b>notion_users</b> nên nhập để request giữ đúng active user/session. Có thể dán riêng value hoặc dạng <code>tên_cookie=value</code>; không dán cả chuỗi Cookie.</div><label>token_v2</label><input id='notion-token-v2' type='password' placeholder='Dán giá trị token_v2' autocomplete='off' spellcheck='false'><label>notion_user_id</label><input id='notion-user-id' placeholder='Dán giá trị notion_user_id' autocomplete='off' spellcheck='false'><label>notion_users</label><input id='notion-users' placeholder='Dán nguyên giá trị notion_users' autocomplete='off' spellcheck='false'><button class='secondary' onclick='notionLogin()'>Đăng nhập bằng Notion session</button><button class='secondary' onclick='loadNotionAccounts()'>Làm mới tài khoản</button><div id='notion-status' class='status hidden'></div><div id='notion-accounts' class='status'>Chưa tải.</div></section></section></main><script>const $=id=>document.getElementById(id);const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const show=(id,text)=>{$(id).textContent=text;$(id).classList.remove('hidden')};const ADMIN_API=location.pathname.startsWith('/admin')?'/admin-api':'/auth';async function api(path,options={}){if(path.startsWith('/auth/'))path=ADMIN_API+path.slice(5);const r=await fetch(path,{...options,headers:{'content-type':'application/json',...(options.headers||{})}});const b=await r.json().catch(()=>({}));if(!r.ok)throw Error(b.detail||b.error?.message||'HTTP '+r.status);return b}function showDashboard(){ $('login-card').classList.add('hidden');$('dashboard').classList.remove('hidden');loadProviders();loadAccounts();loadClients();loadNotionAccounts();const later=window.requestIdleCallback||((fn)=>setTimeout(fn,40));later(()=>{loadOpenRouterStatus();loadTokenRouterStatus();loadNimStatus()})}function showLogin(){ $('login-card').classList.remove('hidden');$('dashboard').classList.add('hidden')}async function check(){try{const r=await api('/auth/me');r.authenticated?showDashboard():showLogin()}catch{showLogin()}}async function login(){try{await api('/auth/login',{method:'POST',body:JSON.stringify({username:$('username').value.trim(),password:$('password').value})});$('password').value='';showDashboard()}catch(e){show('login-status',e.message)}}async function logout(){await api('/auth/logout',{method:'POST'});showLogin()}async function startLogin(){try{const s=await api('/auth/device/start',{method:'POST'});$('code').textContent=s.user_code;$('code').classList.remove('hidden');$('open').classList.remove('hidden');$('open').onclick=()=>window.open(s.verification_url,'_blank');show('device-status','Mở trang xác nhận và nhập mã ở trên. Đang chờ ChatGPT xác nhận…');poll(s.login_id,s.interval_seconds)}catch(e){show('device-status',e.message)}}async function poll(id,interval){for(;;){await new Promise(r=>setTimeout(r,interval*1000));try{const s=await api('/auth/device/poll',{method:'POST',body:JSON.stringify({login_id:id,label:'ChatGPT'})});if(s.status==='completed'){show('device-status','✓ Đăng nhập ChatGPT thành công.');await loadAccounts();return}if(s.status==='expired'||s.status==='failed'){show('device-status','Phiên đăng nhập: '+s.status);return}show('device-status','Đang chờ xác nhận đăng nhập…')}catch(e){show('device-status',e.message);return}}}async function loadAccounts(){try{const r=await api('/auth/accounts');$('accounts').innerHTML=r.data.length?r.data.map(a=>`<div class='account'><span><span class='dot'></span>${esc(a.label)}</span><span>${esc(a.status)}</span></div>`).join(''):'Chưa có tài khoản.'}catch(e){$('accounts').textContent=e.message}}let providersData=null;let providerModelSeq=0;async function loadProviders(){try{const r=await api('/auth/providers');providersData=r;renderProviders(r);const seq=++providerModelSeq;setTimeout(()=>loadProviderModels(r.active_provider,seq),0)}catch(e){$('providers').textContent=e.message}}async function loadProviderModels(provider,seq){try{const r=await api('/auth/providers/'+encodeURIComponent(provider)+'/models');if(seq!==providerModelSeq||!providersData||providersData.active_provider!==provider)return;const active=providersData.providers.find(p=>p.id===provider);if(active)active.models=Array.isArray(r.models)?r.models:[];renderProviderModels(active?active.models:[])}catch{}}const PL={chatgpt:'ChatGPT / Codex',bai:'B.AI',openrouter:'OpenRouter',tokenrouter:'TokenRouter',notion:'Notion AI',nim:'NVIDIA NIM'};function renderProviderModels(models){$('model-options').innerHTML=(models||[]).map(m=>`<option value='${esc(m)}'>`).join('')}function renderProviders(r){$('providers').innerHTML=r.providers.map(p=>`<button class='${p.id===r.active_provider?'':'secondary'}' onclick='selectProvider("${p.id}")'>${esc(PL[p.id]||p.id)}${p.configured?'':' (chưa cấu hình)'}</button>`).join('');const active=r.providers.find(p=>p.id===r.active_provider);renderProviderModels(active&&active.models?active.models:[]);$('model-input').value=r.active_model||'';$('provider-status').textContent='Đang dùng: '+(PL[r.active_provider]||r.active_provider)+(r.active_model?' · model '+r.active_model:' · model mặc định')+(active&&!active.configured?' · chưa cấu hình provider này':'')}async function selectProvider(id){try{await api('/auth/providers/select',{method:'POST',body:JSON.stringify({provider:id})});await loadProviders()}catch(e){$('provider-status').textContent=e.message}}async function selectModel(m){if(!providersData)return;try{await api('/auth/providers/select',{method:'POST',body:JSON.stringify({provider:providersData.active_provider,model:m})});await loadProviders()}catch(e){$('provider-status').textContent=e.message}}async function applyModel(){await selectModel($('model-input').value.trim())}async function loadClients(){try{const r=await api('/auth/clients');$('clients').innerHTML=r.data.length?r.data.map(c=>`<div class='account'><span><span class='dot'></span>${esc(c.label)}<br><small>${esc(c.key_masked)} · ${esc(c.provider)}${c.model?' · '+esc(c.model):''}</small></span><span>${c.status==='active'?`<button class='secondary' style='min-height:36px;width:auto;padding:6px 12px;margin:2px' onclick='clientAction("${c.id}","disabled")'>Tắt</button>`:`<button class='secondary' style='min-height:36px;width:auto;padding:6px 12px;margin:2px' onclick='clientAction("${c.id}","active")'>Bật</button><button class='secondary' style='min-height:36px;width:auto;padding:6px 12px;margin:2px' onclick='clientAction("${c.id}","delete")'>Xóa</button>`}</span></div>`).join(''):'Chưa có client key.'}catch(e){$('clients').textContent=e.message}}async function clientAction(id,action){try{if(action==='delete'){await api('/auth/clients/'+id,{method:'DELETE'})}else{await api('/auth/clients/'+id,{method:'POST',body:JSON.stringify({status:action})})}await loadClients()}catch(e){show('client-created',e.message)}}async function addClient(){try{const r=await api('/auth/clients',{method:'POST',body:JSON.stringify({label:$('client-label').value.trim(),key:$('client-key').value.trim(),provider:$('client-provider').value,model:$('client-model').value.trim()})});$('client-key').value='';$('client-model').value='';show('client-created','✓ Đã tạo. API key client: '+r.key);await loadClients()}catch(e){show('client-created',e.message)}}async function saveOpenRouterKey(){try{await api('/auth/openrouter/key',{method:'POST',body:JSON.stringify({api_key:$('openrouter-key').value.trim()})});$('openrouter-key').value='';show('openrouter-status','✓ Đã lưu OpenRouter API key.');await loadProviders()}catch(e){show('openrouter-status',e.message)}}async function loadOpenRouterStatus(){try{const r=await api('/auth/openrouter/key');if(r.configured)show('openrouter-status','OpenRouter: đã cấu hình.')}catch{}}async function saveTokenRouterKey(){try{await api('/auth/tokenrouter/key',{method:'POST',body:JSON.stringify({api_key:$('tokenrouter-key').value.trim()})});$('tokenrouter-key').value='';show('tokenrouter-status','✓ Đã lưu TokenRouter API key.');await loadProviders()}catch(e){show('tokenrouter-status',e.message)}}async function loadTokenRouterStatus(){try{const r=await api('/auth/tokenrouter/key');if(r.configured)show('tokenrouter-status','TokenRouter: đã cấu hình · '+r.base_url)}catch{}}async function saveNimKey(){try{await api('/auth/nim/key',{method:'POST',body:JSON.stringify({api_key:$('nim-key').value.trim()})});$('nim-key').value='';show('nim-status','✓ Đã lưu NVIDIA NIM API key.');await loadProviders()}catch(e){show('nim-status',e.message)}}async function loadNimStatus(){try{const r=await api('/auth/nim/key');if(r.configured)show('nim-status','NVIDIA NIM: đã cấu hình. Danh sách model tải nền khi chọn provider.')}catch{}}async function startNotionBrowserLogin(){try{const s=await api('/auth/notion/browser/start',{method:'POST',body:JSON.stringify({})});show('notion-browser-status','Chrome/Edge đã được mở trên máy đang chạy gateway. Hãy đăng nhập Notion trong cửa sổ đó; gateway đang chờ hoàn tất…');pollNotionBrowserLogin(s.login_id,s.interval_seconds||2)}catch(e){show('notion-browser-status',e.message)}}async function pollNotionBrowserLogin(id,interval){for(;;){await new Promise(r=>setTimeout(r,interval*1000));try{const s=await api('/auth/notion/browser/poll',{method:'POST',body:JSON.stringify({login_id:id})});if(s.status==='completed'){show('notion-browser-status','✓ Đăng nhập Notion bằng trình duyệt thành công.');await loadNotionAccounts();await loadProviders();return}if(s.status==='failed'||s.status==='expired'){show('notion-browser-status',s.error||('Phiên đăng nhập: '+s.status));return}show('notion-browser-status','Đang chờ bạn đăng nhập Notion trong cửa sổ Chrome/Edge…')}catch(e){show('notion-browser-status',e.message);return}}}async function notionLogin(){const token=$('notion-token-v2').value.trim();const userId=$('notion-user-id').value.trim();const notionUsers=$('notion-users').value.trim();if(!token){show('notion-status','Hãy nhập token_v2.');return}try{await api('/auth/notion/login',{method:'POST',body:JSON.stringify({token_v2:token,notion_user_id:userId,notion_users:notionUsers})});$('notion-token-v2').value='';$('notion-user-id').value='';$('notion-users').value='';show('notion-status','✓ Đăng nhập Notion session thành công.');await loadNotionAccounts();await loadProviders()}catch(e){show('notion-status',e.message)}}async function loadNotionAccounts(){try{const r=await api('/auth/notion/accounts');$('notion-accounts').innerHTML=r.data.length?r.data.map(a=>`<div class='account'><span><span class='dot'></span>${esc(a.label)}<br><small>${esc(a.space_name||a.space_id)}</small></span><span>${a.status==='active'?`<button class='secondary' style='min-height:36px;width:auto;padding:6px 12px;margin:2px' onclick='disableNotion("${a.id}")'>Tắt</button>`:'disabled'}</span></div>`).join(''):'Chưa có tài khoản Notion.'}catch(e){$('notion-accounts').textContent=e.message}}async function disableNotion(id){try{await api('/auth/notion/accounts/'+id,{method:'DELETE'});await loadNotionAccounts();await loadProviders()}catch(e){show('notion-status',e.message)}}check();</script></body></html>"""

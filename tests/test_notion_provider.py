"""Tests for the Notion AI provider (cookie login, NDJSON parsing, chat routing)."""
import json
...
NOTION_COOKIE = "token_v2=tok123; notion_user_id=user-1; notion_browser_id=browser-1"
...
def _signin_notion(monkeypatch) -> TestClient:
    ...
    def fake_bootstrap(runtime_arg, cookie):
        assert "token_v2=" in cookie
        return {
            "token_v2": "tok123",
            "full_cookie": cookie,
            "user_id": "user-1",
    ...
    response = client.post("/auth/notion/login", json={"cookie": NOTION_COOKIE})
...
def test_login_requires_token_v2_cookie(monkeypatch) -> None:
    monkeypatch.setattr(runtime, "require_admin", lambda request: None)
    client = TestClient(app)

    response = client.post("/auth/notion/login", json={"cookie": "notion_user_id=user-1"})
...
def test_notion_list_models_uses_get_available_models(monkeypatch) -> None:
    notion_provider._memory_notion_accounts["acc-1"] = {
        "id": "acc-1", "label": "Test", "cookie_enc": NOTION_COOKIE,

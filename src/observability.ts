...
export async function enforceRateLimit(env: Env, key: string, limit = DEFAULT_RATE_LIMIT): Promise<RateLimitResult> {
  ...
  try {
    ...
    if (requestCount >= limit) return { allowed: false, remaining: 0, resetAt };
    await env.DB.prepare("INSERT INTO rate_limits (bucket, window_start, request_count) VALUES (?, ?, 1) ON CONFLICT(bucket) DO UPDATE SET request_count = request_count + 1").bind(bucket, windowStart).run();
    return { allowed: true, remaining: Math.max(0, limit - requestCount - 1), resetAt };
  } catch {
...
export async function recordUsage(env: Env, route: string, model: string, status: number, latencyMs: number, accountId?: string, clientHash?: string): Promise<void> {
  try {
    const now = Date.now();
    await env.DB.prepare("INSERT INTO usage_events (id, created_at, route, model, status, latency_ms, account_id, client_hash) VALUES (?, ?, ?, ?, ?, ?, ?, ?)").bind(crypto.randomUUID(), now, route, model, status, latencyMs, accountId ?? null, clientHash ?? null).run();
    ...
    // Observability must never take the API path down when its tables are unavailable.
  }
}
...
export async function getUsageSummary(env: Env): Promise<UsageSummary> {
  try {
    ...
    const result = await env.DB.prepare("SELECT COUNT(*) AS total, SUM(CASE WHEN status >= 200 AND status < 400 THEN 1 ELSE 0 END) AS successful, AVG(latency_ms) AS avg_latency_ms FROM usage_events WHERE created_at >= ?").bind(since).first<{ total: number; successful: number; avg_latency_ms: number }>();
    return {
    ...
    return { windowMs: USAGE_RETENTION_MS, totalRequests: 0, successfulRequests: 0, failedRequests: 0, averageLatencyMs: 0 };
  }
}
